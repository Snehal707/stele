...
const CONFIG = {
  chainId: 4221,
  governor: "0x97626F9dD28592cf19A1F73ea61cd8e9Fd1aBbF5",
  ...
  return <main className="product-page">
    ...
    <div className="product-layout wrap">
      ...
      <div className="product-main">
        ...
        {activeProductSection === "chain" && <section className="chain evidence-panel" aria-labelledby="chain-title"><div className="section-intro compact"><div className="eyebrow">05 / CHAIN RECORD</div><p className="scope-note">Primary frozen proof plus the interactive / post-fix v3 deployment.</p></div><div className="chain-list"><article className="chain-record"><div><strong>Primary consolidated Governor</strong><small>Agent C · full halt / govern / lifeform loop</small></div><div><small>GenLayer Bradbury · chain 4221</small></div><div className="chain-address">{CANONICAL_DEMO.governor}</div><a href={`${CONFIG.addressExplorer}${CANONICAL_DEMO.governor}`} target="_blank" rel="noreferrer">explorer ↗</a></article><article className="chain-record"><div><strong>Interactive / post-fix Governor · v3</strong><small>sticky halt, enrollment guard, evidence-conflict safety, trace-based promotion</small></div><div><small>GenLayer Bradbury · chain 4221</small></div><div className="chain-address">{CONFIG.governor}</div><a href={`${CONFIG.addressExplorer}${CONFIG.governor}`} target="_blank" rel="noreferrer">explorer ↗</a></article></div><div className="read-status" role="status"><strong>{readStatus}</strong></div></section>}
