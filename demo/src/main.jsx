...
const HALT_REVERT_PROOF = {
  ...
  hash: "0xd98033826d9737f6598e35cd23b862fe0207a3057f59b3ce163137d4e87ed557",
...
function AlreadyProvedSection({ live, lineage, capital, capitalValue, walletConnected, retryLiveReads }) {
  return <section id="proof" className="already-proved evidence-panel" aria-labelledby="already-proved-title">
    ...
    <article className="proof-status-card">
      ...
      <p className="proof-chain-note">Canonical: <a href={`${CONFIG.addressExplorer}${CANONICAL_DEMO.governor}`} target="_blank" rel="noreferrer">{CANONICAL_DEMO.governor}</a> · Interactive / post-fix: <a href={`${CONFIG.addressExplorer}${CONFIG.governor}`} target="_blank" rel="noreferrer">{CONFIG.governor}</a></p>
    </article>
    ...
    <details className="latest-live-call"><summary><span className="eyebrow">LATEST LIVE CALL · {LATEST_LIVE_REVIEW.date}</span><strong className="proof-on">{LATEST_LIVE_REVIEW.ruling} · later read, not the proof card</strong></summary><div className="latest-live-call-body"><h3 id="latest-live-call-title">Fresh Agent C review on Bradbury</h3><p className="latest-live-call-context">Separate latest live call — this ON_MANDATE result is a later read of a different state, not a replacement for the canonical lifecycle status above.</p><p className="latest-live-call-meta">Governor <code>{LATEST_LIVE_REVIEW.governor}</code> · Agent <code>{LATEST_LIVE_REVIEW.agent}</code></p><p className="latest-live-call-reason">“{LATEST_LIVE_REVIEW.reason}”</p><a className="latest-live-call-hash" href={`${CONFIG.explorer}${LATEST_LIVE_REVIEW.hash}`} target="_blank" rel="noreferrer">{LATEST_LIVE_REVIEW.hash} ↗</a></div></details>
    <HaltRevertProofCard />
    <section className="proof-lineage-block" aria-labelledby="proof-lineage-title"><div className="section-intro compact"><div className="eyebrow">02 / LINEAGE · LIVE READ</div><h3 id="proof-lineage-title">Mandate history and last claim.</h3><p className="scope-note">The live lineage read stays here with the canonical proof instead of competing for a rail slot.</p></div>{lineage.status === "ready" ? <><div className="lineage-rail"><article className="version-card"><div className="version-label">v1 · {lineage.versionOne.status} <EvidenceTag>live · get_mandate_version</EvidenceTag></div><p>{lineage.versionOne.text}</p></article><div className="lineage-arrow" aria-hidden="true">→</div><article className="version-card active-version"><div className="version-label">v2 · {lineage.versionTwo.status} <EvidenceTag>live · get_mandate_version</EvidenceTag></div><p>{renderMandateText(lineage.versionOne.text, lineage.versionTwo.text)}</p></article></div><div className="trigger"><span>CLAIM {lineage.claim.status}</span><b>{String(lineage.claim.payout)} against {String(lineage.claim.loss)} loss <EvidenceTag>live · get_last_claim</EvidenceTag></b><span>CLAUSE APPENDED</span></div></> : <LineageFallback onRetry={retryLiveReads} />}</section>
...
function ProductPage() {
  ...
  return <main className="product-page">
    ...
    <div className="product-wedges wrap" aria-label="Stele in three lines"><p>Caps ask: permitted? We ask: still the job?</p><p>LEASH gates the next key. We halt the vault, pay if wrong, append a clause.</p><p>Courts referee two agents. We cut this vault when it leaves its mandate — nobody holds the switch.</p></div>
    <div className="product-layout wrap">
      ...
      <div className="product-main">
        ...
        {activeProductSection === "actions" && <section id="actions" className="actions evidence-panel" aria-labelledby="actions-title"><div className="section-intro compact"><div className="eyebrow">01 / YOUR RUN</div><p className="scope-note">Interactive story on Governor <code>{CONFIG.governor}</code>. The frozen Agent C lifecycle remains in Already Proved.</p></div><HealthyBurstComparison live={live} /><HaltRevertProofCard /><ActionPanel onResultChange={(result) => setYourRun((previous) => ({ ...previous, [result.action]: result }))} /><div className="your-run-results" aria-label="Your action results"><YourRunResult action="Enroll" result={yourRun.Enroll} /><YourRunResult action="Review" result={yourRun.Review} /><YourRunResult action="Spend" result={yourRun.Spend} /></div></section>}
  ...
  return <main className="docs-page">
    ...
    <div className="docs-layout wrap">
      ...
      <article className="docs-content">
        ...
        <section id="halt"><div className="eyebrow">02 / HALT</div><h2>An OFF ruling changes what the vault can do next.</h2><p>When a review returns OFF_MANDATE, the Governor sets a halt and expiry. VaultTwin checks <code>is_halted(agent)</code> before spending and rejects the payment while the halt is active. Halt is set by an off-mandate or evidence-conflict ruling. It is not cleared by a later ON_MANDATE review. It ends when the enrolled halt window expires.</p><p>Halt expiry uses consensus time, not wall-clock time. The window must cover queue delay and review latency, so Bradbury uses a long protection window.</p></section>
