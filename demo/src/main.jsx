...
function ActionPanel({ onResultChange, onReviewReadRetryReady }) {
  ...
  return <div className="write-panel">
    ...
    <section className="vault-deploy-panel" aria-labelledby="vault-deploy-title">
      ...
      {existingEnrollment.status === "valid" && <div className="vault-deploy-warning" role="status"><strong>This wallet is already enrolled</strong><span>Existing VaultTwin: <code>{existingEnrollment.vault}</code></span><span>The existing agent is ready to review below. Deploying another VaultTwin will not replace this enrollment.</span></div>}
      {existingEnrollment.status === "invalid" && <div className="vault-deploy-error" role="alert"><strong>This wallet is enrolled, but its stored VaultTwin is not usable</strong><span>This enrollment cannot be replaced by this Governor. Use the prepared review fixtures above, or connect a fresh wallet only if you need to create a new enrollment.</span></div>}
