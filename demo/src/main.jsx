...
function ActionPanel({ onResultChange, onReviewReadRetryReady }) {
  ...
  return <div className="write-panel">
    ...
    <section className="vault-deploy-panel" aria-labelledby="vault-deploy-title">
      <div className="vault-deploy-error vault-enrollment-blocked" role="note"><strong>Live self-service enrollment is temporarily unavailable for new wallets</strong><span>on this Governor. <a href="#proof">See Proof for the complete verified evidence</a> — no wallet required.</span></div>
      ...
      {existingEnrollment.status === "checking" && <span role="status">Checking whether this wallet is already enrolled on the current Governor…</span>}
       {existingEnrollment.status === "unavailable" && <div className="vault-deploy-error" role="alert"><strong>Enrollment check unavailable</strong><span>{existingEnrollment.error}</span><span>No enrollment result was received and nothing has been changed on-chain. Retry the check before deploying or enrolling.</span><button type="button" onClick={() => setEnrollmentCheckNonce((nonce) => nonce + 1)}>Retry enrollment check</button></div>}
