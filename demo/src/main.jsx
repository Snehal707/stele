...
function ProductPage() {
  ...
  return <main>
    <header className="product-label wrap"><span>STELE / LIVE EVIDENCE · GENLAYER BRADBURY · CHAIN 4221</span></header>
    <section className="contrast wrap" aria-labelledby="contrast-title"><div className="section-intro compact"><div className="eyebrow">01 / THE CONTRAST</div><h2 id="contrast-title">Healthy versus burst.</h2></div><div className="comparison-grid">{renderCase(cases.healthy, live.healthy)}{renderCase(cases.burst, live.burst)}</div></section>
    <section className="drain wrap" aria-labelledby="drain-title"><div className="section-intro compact"><div className="eyebrow">02 / DRAIN TEST</div><h2 id="drain-title">Same state. Different mandate version.</h2></div><div className="comparison-grid">{renderCase(cases.drainOn, live.drain)}{renderCase(cases.drainOff, live.drain)}</div></section>
    <section className="actions wrap" aria-labelledby="actions-title"><div className="section-intro compact"><div className="eyebrow">03 / LIVE ACTIONS</div><h2 id="actions-title">Inspect it. Then submit a real transaction.</h2></div><ActionPanel /></section>
    <section className="lineage wrap" aria-labelledby="lineage-title"><div className="section-intro compact"><div className="eyebrow">04 / MANDATE LINEAGE</div><h2 id="lineage-title">Stored versions and scoring.</h2></div><div className="lineage-rail"><article className="version-card"><div className="version-label">v1 · superseded</div><p>{MANDATE_V1}</p></article><div className="lineage-arrow" aria-hidden="true">→</div><article className="version-card active-version"><div className="version-label">v2 · active</div><p>{MANDATE_V2}</p></article></div><div className="trigger"><span>CLAIM PAID</span><b>700 against 980 loss</b><span>CLAUSE APPENDED</span></div><p className="consequence"><strong>ON_MANDATE under v1</strong> → <strong>OFF_MANDATE under v2</strong></p></section>
    <section className="cover wrap" aria-labelledby="cover-title"><div className="section-intro compact"><div className="eyebrow">05 / COVER</div><h2 id="cover-title">Claims state.</h2></div><div className="cover-grid"><div><span>POOL</span><strong>{display(capital.pool, "700")}</strong><small>claims pool · live read</small></div><div><span>BOND</span><strong>{display(capital.bond, "1,000")}</strong><small>loss cover before payout</small></div><div><span>LAST CLAIM</span><strong>{claimValue ? `${display(claimValue.payout, "700")} / ${display(claimValue.loss, "980")}` : "700 / 980"}</strong><small>payout / loss · PAID</small></div></div></section>
    <section className="capital wrap" aria-labelledby="capital-title"><div className="section-intro compact"><div className="eyebrow">06 / CAPITAL AND YIELD</div><h2 id="capital-title">LP state.</h2></div><div className="pricing-grid capital-grid"><div><span>LP POOL</span><strong>{display(capital.lpPool, "1,800")}</strong></div><div><span>TOTAL LP SHARES</span><strong>{display(capital.totalShares, "1,500")}</strong></div><div><span>YOUR SHARES</span><strong>{isConnected ? display(capital.yourShares, "0") : "Connect wallet"}</strong></div></div></section>
    <section className="chain wrap" aria-labelledby="chain-title"><div className="section-intro compact"><div className="eyebrow">07 / CHAIN RECORD</div><h2 id="chain-title">Governor provenance.</h2></div><div className="chain-list"><article className="chain-record"><div><strong>Consolidated Governor</strong><small>judgment · halt · cover/lifeform · economics</small></div><div><small>GenLayer Bradbury · chain 4221</small></div><div className="chain-address">{CONFIG.governor}</div><a href={`${CONFIG.addressExplorer}${CONFIG.governor}`} target="_blank" rel="noreferrer">explorer ↗</a></article></div><div className="read-status" role="status"><strong>{readStatus}</strong></div></section>
    <section className="context wrap" aria-labelledby="context-title"><div className="section-intro compact"><div className="eyebrow">SECONDARY EVIDENCE / ALLOWLIST</div><h2 id="context-title">Strangers.</h2></div><div className="context-card"><div className="context-verdict">OFF_MANDATE</div><p>The vault paid two undeclared providers.</p><pre>spend_total=220{"\n"}destination_count=2{"\n"}balance=780{"\n"}destination 0x3333333333333333333333333333333333333333 | declared=no | payments=1 | total=100{"\n"}destination 0x4444444444444444444444444444444444444444 | declared=no | payments=1 | total=120</pre></div></section>
    <section className="pricing wrap" aria-labelledby="pricing-title"><div className="section-intro compact"><div className="eyebrow">SECONDARY EVIDENCE / PRICING</div><h2 id="pricing-title">Reference slice.</h2></div><div className="pricing-grid"><div><span>FILTERED EVENTS</span><strong>380</strong></div><div><span>MEDIAN LOSS</span><strong>$1.6M</strong></div><div><span>P95 LOSS</span><strong>$90.1M</strong></div><div><span>DEMO CAP</span><strong>$1.0M</strong></div><div><span>ILLUSTRATIVE PREMIUM</span><strong>250 bps</strong></div></div><p className="footnote">DeFiLlama, CertiK Hack3D, and Immunefi use different methodologies; their headline totals disagree; we use only the DeFiLlama slice defined in <code>data/pricing.json</code>.</p></section>
