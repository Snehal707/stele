  ...
  return <div className="write-panel">
    ...
    <form className="enroll-panel" onSubmit={enrollNewAgent}>
      ...
      <div className="enroll-form-grid">
        <label>{enrollmentUnavailable ? "Stored VaultTwin address" : "Deployed VaultTwin contract address"}<input value={enrollForm.vault} readOnly={enrollmentUnavailable} onChange={(event) => setEnrollForm((form) => ({ ...form, vault: event.target.value }))} placeholder="0x…" autoComplete="off" /><span>{enrollmentUnavailable ? "Recovered from this wallet's existing Governor enrollment; read-only while enrollment status is being checked." : "This is not your wallet address. It must be a VaultTwin deployed for the current Governor."}</span></label>
